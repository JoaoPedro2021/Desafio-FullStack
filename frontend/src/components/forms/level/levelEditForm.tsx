...
import { useLevelEditForm } from "./useLevelEditForm";

interface FormEditLevelProps {
  children?: React.ReactNode;
  level: Nivel;
};

export const FormEditLevel = ({ children, level }: FormEditLevelProps) => {
        ...
        <div className="flex flex-col">
          <Form {...form}>
            <form onSubmit={onSubmit} className="space-y-8">
              ...
              />
              <Button variant={isPending ? "disabled" : "brand"} type="submit">{isPending ? "Carregando..." : "Editar Nível"}</Button>
            </form>
          </Form>
        </div>
      </DialogContent>
